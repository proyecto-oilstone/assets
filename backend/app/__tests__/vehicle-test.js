...
describe("testing-vehicle-routes", () => {
  ...
  it("GET /api/cars/autos", async () => {
    await api.get("/api/cars/autos").expect(200).expect("Content-Type", /json/);
  });

  it("GET /api/cars/autos/:id", async () => {
    await api.get("/api/cars/autos/1").expect(200).expect("Content-Type", /json/);
  });
  it("PUT /api/cars/editAuto/:id", async () => {
    await api
      .put("/api/cars/editAuto/1")
      .send({
        patente: "ABDASD231",
        año: 1999,
        ProviderId: 1,
  ...
  it("DELETE /api/cars/autos/:id", async () => {
    await api.delete("/api/cars/autos/1").expect(200).expect("Content-Type", /json/);
  });
});
