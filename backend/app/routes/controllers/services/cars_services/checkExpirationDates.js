...
const checkExpirationDates = async () => {
  try {
    ...
    const dischargedNumber = carStates["DISCHARGED"];
    let now = new Date();
    now = now.toISOString().split('T')[0];
    console.log("updating status of cars...");
    const updateStatusSeguro = `UPDATE "Cars" SET status = ${outOfServiceNumber} WHERE "Cars".status != ${dischargedNumber} AND "Cars".id IN 
    ...
    const updateStatusVTV = `UPDATE "Cars" SET status = ${outOfServiceNumber} WHERE "Cars".status != ${dischargedNumber} AND "Cars".id IN 
    ( SELECT c.id FROM "Cars" c 
